// routes/produtos-orcamento.js — Componentes de orçamento (evento, obra, serviço)
import express from "express";
import ProdutoOrcamento from "../models/ProdutoOrcamento.js";
...
router.get("/", async (req, res) => {
  try {
    ...
    const lista = await ProdutoOrcamento.find(filtro).sort({ categoria: 1, nome: 1 }).lean();
    res.json(lista);
  } catch (err) {
...
router.get("/categorias", async (req, res) => {
  try {
    const empresa = req.query.empresa;
    if (!empresa) return res.status(400).json({ error: "empresa obrigatório." });
    const cats = await ProdutoOrcamento.distinct("categoria", { empresa });
...
router.get("/:id", async (req, res) => {
  try {
    const item = await ProdutoOrcamento.findById(req.params.id).lean();
...
router.post("/", async (req, res) => {
  try {
    ...
    const novo = await ProdutoOrcamento.create({
      empresa,
...
router.put("/:id", async (req, res) => {
  try {
    ...
    const atualizado = await ProdutoOrcamento.findByIdAndUpdate(req.params.id, { $set: update }, { new: true }).lean();
...
router.delete("/:id", async (req, res) => {
  try {
    const excluido = await ProdutoOrcamento.findByIdAndDelete(req.params.id);
...
router.post("/importar", async (req, res) => {
  try {
    ...
    const criados = await ProdutoOrcamento.insertMany(
      itens.map((r) => ({
        empresa,
