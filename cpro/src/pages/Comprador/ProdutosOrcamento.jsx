// ProdutosOrcamento.jsx — Componentes para orçamentos (evento, obra, serviço)
...
export default function ProdutosOrcamento() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      fetch(`${API_URL}/api/produtos-orcamento/categorias?empresa=${encodeURIComponent(empresaId)}`).then((r) => r.json()).catch(() => []),
    ])
      .then(([lista, cats]) => {
  ...
  async function salvar() {
    ...
    try {
      if (editId) {
        await fetch(`${API_URL}/api/produtos-orcamento/${editId}`, {
          method: "PUT",
        ...
        await fetch(`${API_URL}/api/produtos-orcamento`, {
          method: "POST",
          headers: { "Content-Type": "application/json" },
      ...
      const lista = await fetch(`${API_URL}/api/produtos-orcamento?empresa=${encodeURIComponent(empresaId)}`).then((r) => r.json());
      setItens(Array.isArray(lista) ? lista : []);
  ...
  async function excluir(item) {
    ...
    try {
      await fetch(`${API_URL}/api/produtos-orcamento/${item._id}`, { method: "DELETE" });
  ...
  async function importarArquivo(e) {
    ...
    try {
      ...
      const res = await fetch(`${API_URL}/api/produtos-orcamento/importar`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ empresa: empresaId, itens: rows }) });
    ...
    <div className="layout-content-inner" style={{ width: "100%", padding: 0, boxSizing: "border-box", color: "#e6edf3" }}>
      <main style={{ margin: "24px 0", padding: "0 20px 40px" }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 16, marginBottom: 24 }}>
          <div>
            <h2 style={{ margin: 0, color: "#e6edf3", fontSize: "1.5rem" }}>Produtos</h2>
            <p style={{ margin: "4px 0 0", color: "#8b949e", fontSize: "0.9375rem" }}>Itens para propostas personalizadas: eventos, obras, serviços. Importe planilhas.</p>
          </div>
          <div style={{ display: "flex", gap: 8 }}>
            <input type="file" accept=".xlsx,.xls,.csv" onChange={importarArquivo} style={{ display: "none" }} id="importar-prod-orc" />
            <button type="button" onClick={() => document.getElementById("importar-prod-orc").click()} style={btnSec}>Importar</button>
            <button type="button" onClick={() => setMostrarColar(!mostrarColar)} style={btnSec}>Colar texto</button>
            <button type="button" onClick={() => navigate("/orcamentos")} style={btnPrim}>Orçamentos</button>
          </div>
        </div>
